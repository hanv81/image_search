...
imageList = list(paths.list_images('images'))
...
for i,keypoint in enumerate(keypoints):
    ...
    filepath = os.path.join("keypoints", os.path.basename(imageList[i]).split('.')[0] + '.txt')
...
for i,descriptor in enumerate(descriptors):
    filepath = os.path.join("descriptors", os.path.basename(imageList[i]).split('.')[0] + ".txt")
    with open(filepath, 'wb') as fp:
